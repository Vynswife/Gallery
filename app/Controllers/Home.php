    ...
    public function __construct()
{
    ...
    if(session()->get('level') > 0) {
        ...
        // Fetch the number of albums
        $data['total_album'] = $model->getSuratCount('album');

        // Fetch the number of photos
        $data['total_foto'] = $model->getSuratCount('foto');
    ...
    $data['setting'] = $model->getWhere('toko', $where);
        // Fetch the latest photos or albums for display in the gallery
...
public function album()
{
    if (session()->get('level') > 0) {
        ...
        $data['likes'] = $model->getSuratCount('likes');

        // Fetch all albums
        $data['albums'] = $model->tampil('album');

        // Fetch photos and associated comments for each album
        foreach ($data['albums'] as $album) {
            $album->photos = $model->tampil_join2('album', 'foto', 'foto.id_album = album.id_album', ['album.id_album' => $album->id_album]);

            foreach ($album->photos as $photo) {
                // Get like count for each photo
                $photo->like_count = $model->db->table('like')->where('id_foto', $photo->id_foto)->countAllResults();

                // Fetch comments for each photo
                $photo->comments = $model->db->table('komentar')
                                              ->join('user', 'user.id_user = komentar.id_user')
                                              ->where('id_foto', $photo->id_foto)
                                              ->get()->getResult();
            }
        }
        ...
        echo view('header');
        echo view('menu', $data);
        echo view('rak_surat', $data);  // Render the view with albums, photos, and comments
    } else {
...
public function like()
{
    ...
    $idUser = session()->get('id_user'); // Get the logged-in user ID

    // Prepare the data for the "like" record
    $likeData = [
        'id_foto' => $idFoto,
        'id_user' => $idUser,
        'tanggal_like' => date('Y-m-d H:i:s'), // Get the current timestamp
    ];

    // Initialize the model
    $model = new M_burger();

    // Insert the like data into the "like" table
    $model->tambah('likes',$likeData);

    // Optionally, you can check if the insertion was successful and handle any errors here.

    // Set flash data to show a success message
    session()->setFlashdata('message', 'You liked the photo!');

    // Redirect the user to the album page or another page of your choice
    return redirect()->to(base_url('home/album'));
}
...
public function comment()
{
    ...
    // Prepare the data for the comment record
    $commentData = [
        'id_foto' => $idFoto,
        'id_user' => $idUser,
        'isi_komen' => $commentText,
        'tanggal_komentar' => date('Y-m-d H:i:s'), // Get the current timestamp
    ];
    ...
    $model->tambah('komentar', $commentData);

    // Optionally, you can check if the insertion was successful and handle any errors here.

    // Set flash data to show a success message
    session()->setFlashdata('message', 'Your comment has been posted!');
...
public function photo()
{
    if(session()->get('level') > 0) {
    $model= new M_burger;
    $data['jel'] = $model->tampil_join1('foto', 'user', 'foto.id_user = user.id_user');
    $data['jes'] = $model->tampilgambar('toko'); // Mengambil data dari tabel 'toko'
    $data['folders'] = $model->tampil('album'); 
...
public function aksi_t_register()
{
    ...
    $jk = $this->request->getPost('jk'); // Assuming this is for gender, but you didn't mention if it goes into the DB.
